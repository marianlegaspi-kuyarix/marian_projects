...
canvas = tkinter.Canvas(window, bg = "magenta", width = WINDOW_WIDTH, height = WINDOW_HEIGHT, borderwidth = 0, highlightthickness = 0)
canvas.pack()
window.update()
...
def draw():
    ...
    if (game_over):
        canvas.create_text(WINDOW_WIDTH/2, WINDOW_HEIGHT/2, font = ('Helvetica','30','bold'), text = f"GAME OVER: {score}", fill = "black")
    else:
        canvas.create_text(30,20, font = "Arial 10", text = f"SCORE: {score}", fill = "black")
    #how fast the snake would go
    window.after(100, draw) #100ms = 1/10 frames/seconds
